...
extern "C" void* Engine_ProcessFrame(Engine* engine, void* inputTexture, void* cmdBuf);
extern "C" void* Engine_GenerateInterpolatedFrame(Engine* engine, void* prevTex, 
                                                   void* currTex, float t, void* cmdBuf);
extern "C" void Engine_PushFrame(Engine* engine, void* texture, double timestamp);
...
struct DirectFrameEngineOpaque {
    ...
    size_t getMaxFrameBuffer() const {
        return config.reduceLatency ? kMaxFrameBufferLowLatency : kMaxFrameBufferNormal;
    }
    ...
    DirectFrameEngineOpaque()
        ...
        , renderEncoderCount(0)
        , computeEncoderCount(0)
        , blitEncoderCount(0)
        , commandBufferCount(0)
        , drawCallCount(0)
        , fpsFrameCount(0)
        , lastCaptureTimestamp(0.0)
        , state(DirectEngineStateIdle)
        , debugMode(false)
        ...
        , interpolationT(0.0f)
        , frameGenCycle(0)
        , asyncFrameGenQueue(nullptr)
        , asyncFrameGenEnabled(false)
        , pendingInterpolatedFrames(0)
        , generatedFramesThisCycle(0)
        , asyncWriteIndex(0)
        , asyncReadIndex(0)
        , frameInterval(16.67)
        , lastInterpolationTime(0.0)
        , targetFrameMultiplier(2)
        , cachedOutputTex(nil)
        , cachedPrevTex(nil)
        , cachedInterpTex(nil)
        , cachedInputTexture(nil)
        , cachedPerformancePipeline(nil)
        , cachedBalancedPipeline(nil)
        , cachedQualityPipeline(nil)
        , cachedMotionEstPipeline(nil)
        , cachedMotionEstOptPipeline(nil)
        , cachedMotionRefinePipeline(nil)
        , cachedAdaptiveInterpPipeline(nil)
        , cachedMotionVectorTex(nil)
        , cachedConfidenceTex(nil)
        , cachedWidth(0)
        , cachedHeight(0)
        , opticalFlowPipeline(nil)
        , flowTexture(nil)
        , frameGenPipeline(nil)
    ...
    ~DirectFrameEngineOpaque() {
        cleanupFrameBuffer();
        if (currentSurface) CFRelease(currentSurface);
        if (previousSurface) CFRelease(previousSurface);
        if (currentPixelBuffer) CVPixelBufferRelease(currentPixelBuffer);
        if (previousPixelBuffer) CVPixelBufferRelease(previousPixelBuffer);
    ...
    void cleanupFrameBuffer() {
        ...
        for (auto& entry : frameBuffer) {
            if (entry.surface) CFRelease(entry.surface);
        }
        frameBuffer.clear();
    }
    ...
    void pushFrame(IOSurfaceRef surface, double captureTimestamp = 0.0) {
        ...
        while (frameBuffer.size() >= maxBuffer) {
            ...
            if (oldest.surface) CFRelease(oldest.surface);
        ...
        if (captureTimestamp > 0.0) {
            ...
            captureLatency.store((float)((now - captureTimestamp) * 1000.0), std::memory_order_relaxed);
        }
    }
    ...
    void updateFPS() {
        ...
        if (config.frameGenMode != DirectFrameGenOff) {
            ...
            interpolatedFPS.store(currentFPS.load(std::memory_order_relaxed), std::memory_order_relaxed);
        }
    }
    
    void initAsyncFrameGen() {
        if (asyncFrameGenQueue) return;
        if (!device) {
            NSLog(@"MetalGoose: Cannot initialize async frame gen - device is nil");
            return;
        }
        
        dispatch_queue_attr_t attr = dispatch_queue_attr_make_with_qos_class(
            DISPATCH_QUEUE_SERIAL, QOS_CLASS_USER_INTERACTIVE, 0);
        asyncFrameGenQueue = dispatch_queue_create("com.metalgoose.asyncframegen", attr);
        
        if (!asyncFrameGenQueue) {
            NSLog(@"MetalGoose: Failed to create async frame gen queue");
            return;
        }
        ...
        if (debugMode) {
            NSLog(@"MetalGoose: Async frame generation initialized with CPU motion estimation");
        }
    }
    ...
    void ensureAsyncBuffers(size_t width, size_t height) {
        ...
        for (int i = 0; i < 3; i++) {
            if (!asyncOutputTextures[i] || 
                asyncOutputTextures[i].width != width || 
                asyncOutputTextures[i].height != height) {
                asyncOutputTextures[i] = [device newTextureWithDescriptor:desc];
                asyncOutputTextures[i].label = [NSString stringWithFormat:@"AsyncOutput%d", i];
            }
        }
        
        {
            ...
            if (cpuMotionCache.width != mvWidth || cpuMotionCache.height != mvHeight) {
    ...
    void computeCPUMotionVectors(const uint8_t* prevData, const uint8_t* currData,
        ...
        dispatch_apply(mvHeight, dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^(size_t by) {
            for (size_t bx = 0; bx < mvWidth; bx++) {
                        ...
                        for (int py = -2; py <= 2; py++) {
                            for (int px = -2; px <= 2; px++) {
                                ...
                                if (sx >= 0 && sx < (int)width && sy >= 0 && sy < (int)height &&
                                    tx >= 0 && tx < (int)width && ty >= 0 && ty < (int)height) {
                ...
                cpuMotionCache.confidence[mvIdx] = fmaxf(0.0f, 1.0f - bestSAD / 128.0f);
            }
        });
    ...
    void uploadMotionToGPU(id<MTLCommandBuffer> commandBuffer) {
        ...
        if (!cpuMotionCache.valid || !cachedMotionVectorTex) return;
        
        size_t dataSize = cpuMotionCache.width * cpuMotionCache.height * 4 * sizeof(float);
        ...
        if (blit) {
            [blit copyFromBuffer:stagingBuffer
             ...
             sourceBytesPerImage:dataSize
                      sourceSize:MTLSizeMake(cpuMotionCache.width, cpuMotionCache.height, 1)
    ...
    void scheduleAsyncInterpolation(id<MTLTexture> prevTex, id<MTLTexture> currTex, 
                                     int frameMultiplier) {
        if (!asyncFrameGenEnabled.load(std::memory_order_acquire)) return;
        
        targetFrameMultiplier.store(frameMultiplier, std::memory_order_release);
        
        double currentTime = CACurrentMediaTime();
        double prevTime = lastInterpolationTime.exchange(currentTime, std::memory_order_relaxed);
        if (prevTime > 0) {
    ...
    id<MTLTexture> getNextAsyncOutputTexture() {
        ...
        if (readIdx == writeIdx) return nil;
    ...
    void setupPipelines() {
        ...
        auto createPipeline = [&](NSString* name) -> id<MTLComputePipelineState> {
            ...
            if (!func) {
                if (debugMode) {
                    NSLog(@"MetalGoose: Function %@ not found in library", name);
                }
                return nil;
            }
            ...
            if (error) {
                NSLog(@"MetalGoose: Pipeline creation error for %@: %@", name, error.localizedDescription);
                return nil;
            }
            return pipeline;
        };
        ...
        cachedAdaptiveInterpPipeline = createPipeline(@"mgfg1AdaptiveInterpolation");
        
        cachedMotionEstPipeline = createPipeline(@"mgfg1MotionEstimation");
        cachedMotionEstOptPipeline = createPipeline(@"mgfg1MotionEstimationOptimized");
        ...
        if (debugMode) {
            NSLog(@"MetalGoose: Pipelines initialized - Performance:%@ Balanced:%@ Quality:%@",
                  cachedPerformancePipeline ? @"OK" : @"FAIL",
                  cachedBalancedPipeline ? @"OK" : @"FAIL",
                  cachedQualityPipeline ? @"OK" : @"FAIL");
            NSLog(@"MetalGoose: OpticalFlow:%@ FrameGen:%@ Scale:%@",
                  opticalFlowPipeline ? @"OK" : @"FAIL",
                  frameGenPipeline ? @"OK" : @"FAIL",
                  scalePipeline ? @"OK" : @"FAIL");
        }
    }
    
    void ensureTextures(size_t width, size_t height) {
        if (width == cachedWidth && height == cachedHeight && cachedOutputTex != nil) {
            return;
        }
        ...
        if (debugMode) {
            NSLog(@"MetalGoose: Textures created at %zux%zu", width, height);
            NSLog(@"MetalGoose: flowTexture=rg16Float, previousFrame/currentFrame=rgba16Float, output=bgra8Unorm");
        }
    }
    ...
    id<MTLComputePipelineState> getInterpolationPipeline() {
        switch (config.frameGenQuality) {
            ...
            case DirectFrameGenQualityQuality:
                return cachedQualityPipeline ? cachedQualityPipeline : cachedAdaptiveInterpPipeline;
            default:
                return cachedBalancedPipeline;
        }
    }
...
@interface DirectEngineStreamOutput : NSObject <SCStreamOutput, SCStreamDelegate>
    ...
    ofType:(SCStreamOutputType)type 
{
    ...
    auto now = std::chrono::high_resolution_clock::now();
    float elapsedTime = std::chrono::duration<float>(now - engine->lastFrameTime).count();
    engine->lastFrameTime = now;
    
    if (elapsedTime > 0.001f && elapsedTime < 1.0f) {
        float instantFPS = 1.0f / elapsedTime;
        float currentCaptureFPS = engine->captureFPS.load(std::memory_order_relaxed);
        
        if (currentCaptureFPS < 1.0f) {
            engine->captureFPS.store(instantFPS, std::memory_order_relaxed);
        } else {
            float alpha = 0.15f;
            engine->captureFPS.store(currentCaptureFPS * (1.0f - alpha) + instantFPS * alpha, 
        ...
        if (engine->config.frameGenMode != DirectFrameGenOff) {
            ...
            engine->interpolatedFPS.store(engine->captureFPS.load(), std::memory_order_relaxed);
        }
    }
...
- (void)stream:(SCStream*)stream didStopWithError:(NSError*)error {
    if (error && self.engine) {
        ...
        if (self.engine->debugMode) {
            NSLog(@"MetalGoose: Stream stopped with error: %@", error.localizedDescription);
        }
    }
}
...
DirectEngineRef DirectEngine_Create(id<MTLDevice> device, id<MTLCommandQueue> queue) {
    if (!device || !queue) {
        NSLog(@"MetalGoose: DirectEngine_Create failed - invalid device or queue");
        return nullptr;
    }
    ...
    if (status != kCVReturnSuccess || !engine->textureCache) {
        NSLog(@"MetalGoose: Failed to create CVMetalTextureCache: %d", status);
        delete engine;
        return nullptr;
    }
    ...
    if (!engine->cppEngine) {
        NSLog(@"MetalGoose: Failed to create C++ engine core");
    ...
    NSLog(@"MetalGoose: Engine created with async frame gen on %@", device.name);
...
void DirectEngine_SetConfig(DirectEngineRef engine, DirectEngineConfig config) {
    if (!engine || !engine->cppEngine) return;
    ...
    if (engine->captureStream && engine->streamConfig) {
        ...
        [engine->captureStream updateConfiguration:streamConfig completionHandler:^(NSError* error) {
            if (error) {
...
bool DirectEngine_SetTargetDisplay(DirectEngineRef engine, CGDirectDisplayID displayID) {
    if (!engine) return false;
...
bool DirectEngine_StartCapture(DirectEngineRef engine) {
    if (!engine) return false;
    if (engine->state.load(std::memory_order_relaxed) == DirectEngineStateCapturing) {
        return true;
    }
    ...
    switch (engine->config.renderScale) {
        case DirectRenderScale75: renderScaleFactor = 0.75f; break;
        case DirectRenderScale67: renderScaleFactor = 0.67f; break;
        case DirectRenderScale50: renderScaleFactor = 0.50f; break;
        case DirectRenderScale33: renderScaleFactor = 0.33f; break;
        default: renderScaleFactor = 1.0f; break;
    }
    ...
    [SCShareableContent getShareableContentWithCompletionHandler:^(SCShareableContent *content, NSError *error) {
        ...
        if (useWindow && targetWID != kCGNullWindowID) {
            ...
            if (targetWindow) {
                size_t baseW = engine->config.baseWidth > 0 ? (size_t)engine->config.baseWidth : (size_t)targetWindow.frame.size.width;
                size_t baseH = engine->config.baseHeight > 0 ? (size_t)engine->config.baseHeight : (size_t)targetWindow.frame.size.height;
                config.width = std::max<size_t>(1, (size_t)std::llround((double)baseW * (double)renderScaleFactor));
                config.height = std::max<size_t>(1, (size_t)std::llround((double)baseH * (double)renderScaleFactor));
            ...
            size_t baseW = engine->config.baseWidth > 0 ? (size_t)engine->config.baseWidth : (size_t)selected.width;
            size_t baseH = engine->config.baseHeight > 0 ? (size_t)engine->config.baseHeight : (size_t)selected.height;
            config.width = std::max<size_t>(1, (size_t)std::llround((double)baseW * (double)renderScaleFactor));
            config.height = std::max<size_t>(1, (size_t)std::llround((double)baseH * (double)renderScaleFactor));
...
DirectEngineState DirectEngine_GetState(DirectEngineRef engine) {
    return engine ? engine->state.load(std::memory_order_relaxed) : DirectEngineStateError;
}
...
id<MTLTexture> DirectEngine_GetFrameTexture(DirectEngineRef engine, id<MTLDevice> device) {
    if (!engine || !device) return nil;
...
id<MTLTexture> DirectEngine_ProcessFrame(DirectEngineRef engine, 
    ...
    if (!engine || !engine->cppEngine || !device || !commandBuffer) return nil;
    ...
    id<MTLTexture> inputTexture = [device newTextureWithDescriptor:desc 
    ...
    if (engine->config.frameGenMode != DirectFrameGenOff && 
        ...
        dispatch_async(engine->asyncFrameGenQueue, ^{
            ...
            const uint8_t* prevData = (const uint8_t*)IOSurfaceGetBaseAddress(prevSurf);
            const uint8_t* currData = (const uint8_t*)IOSurfaceGetBaseAddress(currSurf);
            ...
            if (engine->debugMode) {
                NSLog(@"MetalGoose: CPU motion estimation complete for %zux%zu", w, h);
            }
        });
    }
    ...
    if (currentGpuTime < 0.1f) {
        ...
        engine->gpuTime.store(currentGpuTime * 0.9f + processingMs * 0.8f * 0.1f, std::memory_order_relaxed);
    }
...
id<MTLTexture> DirectEngine_GetInterpolatedFrame(DirectEngineRef engine, 
                                                 id<MTLDevice> device, 
                                                 id<MTLCommandBuffer> commandBuffer, 
                                                 bool* isInterpolated) {
    return DirectEngine_GetInterpolatedFrameWithT(engine, device, commandBuffer, 0.5f, isInterpolated);
}

id<MTLTexture> DirectEngine_GetInterpolatedFrameWithT(DirectEngineRef engine, 
                                                      id<MTLDevice> device, 
                                                      id<MTLCommandBuffer> commandBuffer, 
                                                      float t,
                                                      bool* isInterpolated) {
    if (!engine || !device || !commandBuffer) {
        if (isInterpolated) *isInterpolated = false;
        return nil;
    }
    ...
    *isInterpolated = false;
    ...
    if (engine->config.frameGenMode == DirectFrameGenOff) {
        if (engine->debugMode) NSLog(@"MetalGoose: Frame gen disabled");
        return nil;
    }
    
    if (!engine->currentSurface) {
        if (engine->debugMode) NSLog(@"MetalGoose: No current surface");
        return nil;
    }
    
    IOSurfaceRef prevSurface = engine->previousSurface ? engine->previousSurface : engine->currentSurface;
    ...
    if (width == 0 || height == 0) {
        if (engine->debugMode) NSLog(@"MetalGoose: Invalid surface dimensions");
        return nil;
    }
    ...
    if (!currentTex || !previousTex) {
        if (engine->debugMode) NSLog(@"MetalGoose: Failed to create textures from surfaces");
        return nil;
    }
    ...
    if (engine->cppEngine) {
        void* cppResult = Engine_GenerateInterpolatedFrame(
            engine->cppEngine,
            (__bridge void*)previousTex,
            (__bridge void*)currentTex,
            t,
    ...
    bool useExternalMotionEstimation = (engine->config.frameGenQuality == DirectFrameGenQualityQuality);
    ...
    if (useCPUMotionCache && engine->cachedMotionVectorTex) {
        ...
        if (engine->debugMode) {
            NSLog(@"MetalGoose: Using CPU-computed motion vectors (hybrid mode)");
        }
    } else if (useExternalMotionEstimation && engine->cachedMotionEstOptPipeline && engine->cachedMotionVectorTex) {
        id<MTLComputeCommandEncoder> motionEncoder = [commandBuffer computeCommandEncoder];
        if (motionEncoder) {
            ...
            [motionEncoder setComputePipelineState:engine->cachedMotionEstOptPipeline];
            ...
            [motionEncoder setBytes:&mvParams length:sizeof(MGFG1ParamsBridge) atIndex:0];
            
            MTLSize motionThreadGroupSize = MTLSizeMake(8, 8, 1);
            MTLSize motionGridSize = MTLSizeMake((width + 7) / 8, (height + 7) / 8, 1);
            [motionEncoder dispatchThreadgroups:motionGridSize threadsPerThreadgroup:motionThreadGroupSize];
            ...
            if (engine->config.frameGenQuality == DirectFrameGenQualityQuality && 
                ...
                if (refineEncoder) {
                    ...
                    [refineEncoder setComputePipelineState:engine->cachedMotionRefinePipeline];
                    ...
                    [refineEncoder dispatchThreadgroups:motionGridSize threadsPerThreadgroup:motionThreadGroupSize];
    ...
    *isInterpolated = true;
    ...
    if (engine->debugMode) {
        NSLog(@"MetalGoose: Interpolated frame %llu with t=%.2f", 
              engine->interpolatedFrameCount.load(), t);
    }
    
    return engine->cachedOutputTex;
}

id<MTLTexture> DirectEngine_InterpolateTextures(DirectEngineRef engine, 
                                                 id<MTLDevice> device, 
                                                 id<MTLCommandBuffer> commandBuffer,
                                                 id<MTLTexture> prevTexture,
                                                 id<MTLTexture> currTexture,
                                                 float t) {
    ...
    bool useMotionCompensation = (engine->config.frameGenQuality == DirectFrameGenQualityQuality ||
                                   engine->config.frameGenQuality == DirectFrameGenQualityBalanced);
    
    if (useMotionCompensation && engine->cachedMotionEstOptPipeline && engine->cachedMotionVectorTex) {
        id<MTLComputeCommandEncoder> motionEncoder = [commandBuffer computeCommandEncoder];
        if (motionEncoder) {
...
int DirectEngine_GetEffectiveMultiplier(DirectEngineRef engine) {
    if (!engine) return 1;
    if (engine->config.frameGenMode == DirectFrameGenOff) return 1;
    
    if (engine->config.frameGenType == DirectFrameGenTypeAdaptive) {
        float captureFPS = engine->captureFPS.load(std::memory_order_relaxed);
        if (captureFPS < 10.0f) captureFPS = 30.0f;
...
void DirectEngine_SetTargetFrameMultiplier(DirectEngineRef engine, int multiplier) {
    if (!engine) return;
...
static uint64_t calculateTextureMemory(DirectFrameEngineOpaque* engine) {
    if (!engine || engine->cachedWidth == 0 || engine->cachedHeight == 0) return 0;
    ...
    int bgra8Count = 0;
    if (engine->cachedOutputTex) bgra8Count++;
    if (engine->cachedPrevTex) bgra8Count++;
    if (engine->cachedInterpTex) bgra8Count++;
    if (engine->cachedInputTexture) bgra8Count++;
    if (engine->finalOutputTexture) bgra8Count++;
    totalBytes += bgra8Size * bgra8Count;
    
    size_t rgba16fSize = w * h * 8;
    int rgba16fCount = 0;
    if (engine->previousFrameTexture) rgba16fCount++;
    if (engine->currentFrameTexture) rgba16fCount++;
    if (engine->interpolatedTexture) rgba16fCount++;
    if (engine->scaledTexture) rgba16fCount++;
    if (engine->cachedMotionVectorTex) rgba16fCount++;
    totalBytes += rgba16fSize * rgba16fCount;
    
    size_t rg16fSize = w * h * 4;
    if (engine->flowTexture) totalBytes += rg16fSize;
    
    size_t r16fSize = w * h * 2;
    if (engine->cachedConfidenceTex) totalBytes += r16fSize;
...
DirectEngineStats DirectEngine_GetStats(DirectEngineRef engine) {
    ...
    stats.interpolatedFPS = engine->interpolatedFPS.load(std::memory_order_relaxed);
    if (stats.interpolatedFPS <= 0.0f && engine->config.frameGenMode != DirectFrameGenOff) {
        float baseFPS = (captureFPS > 0.0f) ? captureFPS : 30.0f;
        if (engine->config.frameGenType == DirectFrameGenTypeAdaptive) {
            stats.interpolatedFPS = static_cast<float>(engine->config.adaptiveTargetFPS);
        } else {
            stats.interpolatedFPS = baseFPS * static_cast<float>(engine->config.frameGenMultiplier);
        }
    ...
    stats.interpolatedFrameCount = engine->interpolatedFrameCount.load(std::memory_order_relaxed);
    ...
    stats.renderEncoders = engine->renderEncoderCount.load(std::memory_order_relaxed);
    stats.computeEncoders = engine->computeEncoderCount.load(std::memory_order_relaxed);
    stats.blitEncoders = engine->blitEncoderCount.load(std::memory_order_relaxed);
    stats.commandBuffers = engine->commandBufferCount.load(std::memory_order_relaxed);
...
NSString* DirectEngine_GetLastError(DirectEngineRef engine) {
    if (!engine || engine->lastError.empty()) return nil;
...
bool DirectEngine_SetOutputDisplay(DirectEngineRef engine, CGDirectDisplayID displayID) {
    if (!engine) return false;
...
NSString* DirectEngine_GetDebugInfo(DirectEngineRef engine) {
    ...
    switch (engine->state.load()) {
        case DirectEngineStateIdle: stateStr = @"Idle"; break;
        case DirectEngineStateCapturing: stateStr = @"Capturing"; break;
        case DirectEngineStateProcessing: stateStr = @"Processing"; break;
        case DirectEngineStatePresenting: stateStr = @"Presenting"; break;
        case DirectEngineStateError: stateStr = @"Error"; break;
        default: stateStr = @"Unknown";
    }
    
    return [NSString stringWithFormat:
        @"MetalGoose Engine Debug Info\n"
        @"============================\n"
        @"State: %@\n"
        @"Device: %@\n"
        @"Capture FPS: %.1f\n"
