...
public:
    ...
    void clear() { count_ = 0; writeIdx_ = 0; }
...
public:
    ...
    uint64_t getFrameIndex() const { return frameIndex_.load(std::memory_order_relaxed); }
    
    id<MTLTexture> processFrame(id<MTLTexture> inputTexture, id<MTLCommandBuffer> commandBuffer);
    id<MTLTexture> generateInterpolatedFrame(id<MTLTexture> prevTexture, id<MTLTexture> currTexture, 
                                              float t, id<MTLCommandBuffer> commandBuffer);
    
    id<MTLTexture> applyAntiAliasing(id<MTLTexture> inputTexture, id<MTLCommandBuffer> commandBuffer);
    id<MTLTexture> applySharpening(id<MTLTexture> inputTexture, id<MTLCommandBuffer> commandBuffer);
    id<MTLTexture> applyTAA(id<MTLTexture> inputTexture, id<MTLCommandBuffer> commandBuffer);
    
    float getProcessingTime() const { return processingTime_.load(std::memory_order_relaxed); }
...
private:
    ...
    void ensureScaler(size_t inputWidth, size_t inputHeight, size_t outputWidth, size_t outputHeight);
    
    void computeMotionVectors(id<MTLTexture> prevTex, id<MTLTexture> currTex, id<MTLCommandBuffer> commandBuffer);
    void computeMotionVectorsPyramid(id<MTLTexture> prevTex, id<MTLTexture> currTex, id<MTLCommandBuffer> commandBuffer);
    id<MTLTexture> interpolateWithMotion(id<MTLTexture> prevTex, id<MTLTexture> currTex, float t, id<MTLCommandBuffer> commandBuffer);
    id<MTLTexture> interpolateSimple(id<MTLTexture> prevTex, id<MTLTexture> currTex, float t, id<MTLCommandBuffer> commandBuffer);
    id<MTLTexture> interpolateBalanced(id<MTLTexture> prevTex, id<MTLTexture> currTex, float t, id<MTLCommandBuffer> commandBuffer);
    void copyTextureToHistory(id<MTLTexture> source, id<MTLCommandBuffer> commandBuffer);
...
Engine::Engine(id<MTLDevice> device, id<MTLCommandQueue> queue)
    : device_(device)
    , queue_(queue)
    , motionEstimationPipeline_(nil)
    , motionEstimationOptimizedPipeline_(nil)
    , motionRefinementPipeline_(nil)
    , pyramidDownsample2xPipeline_(nil)
    , pyramidDownsample4xPipeline_(nil)
    , pyramidMotionPipeline_(nil)
    , upsampleMotionPipeline_(nil)
    , performancePipeline_(nil)
    , balancedPipeline_(nil)
    , qualityPipeline_(nil)
    , adaptivePipeline_(nil)
    , fxaaPipeline_(nil)
    , casPipeline_(nil)
    , usmPipeline_(nil)
    ...
    , outputTexture_(nil)
    , tempTexture_(nil)
    , motionVectorTexture_(nil)
    , confidenceTexture_(nil)
    , historyTexture_(nil)
    , previousFrameTexture_(nil)
    , taaOutputTexture_(nil)
    , pyramidPrevLevel1_(nil)
    , pyramidPrevLevel2_(nil)
    , pyramidCurrLevel1_(nil)
    , pyramidCurrLevel2_(nil)
    , motionLevel1_(nil)
    , motionLevel2_(nil)
...
void Engine::setupPipelines() {
    ...
    auto createPipeline = [&](NSString* name) -> id<MTLComputePipelineState> {
        ...
        if (error) {
            NSLog(@"Pipeline creation error for %@: %@", name, error.localizedDescription);
            return nil;
        }
        return pipeline;
    };
    ...
    motionEstimationOptimizedPipeline_ = createPipeline(@"mgfg1MotionEstimationOptimized");
...
void Engine::ensureTextures(size_t width, size_t height) {
    if (width == cachedWidth_ && height == cachedHeight_ && outputTexture_ != nil) {
        return;
    }
...
void Engine::ensureMotionTextures(size_t width, size_t height) {
    if (motionVectorTexture_ != nil && cachedWidth_ == width && cachedHeight_ == height) {
        return;
    }
...
void Engine::ensurePyramidTextures(size_t width, size_t height) {
    ...
    if (pyramidPrevLevel1_ != nil && 
        pyramidPrevLevel1_.width == level1Width && 
...
void Engine::ensureScaledOutputTexture(size_t outputWidth, size_t outputHeight) {
    if (scaledOutputTexture_ && scalerOutputWidth_ == outputWidth && scalerOutputHeight_ == outputHeight) {
        return;
    }
    ...
    MTLTextureDescriptor* outDesc = [MTLTextureDescriptor
        ...
        height:outputHeight
        mipmapped:NO];
    outDesc.usage = MTLTextureUsageShaderRead | MTLTextureUsageShaderWrite | MTLTextureUsageRenderTarget;
    outDesc.storageMode = MTLStorageModePrivate;
    scaledOutputTexture_ = [device_ newTextureWithDescriptor:outDesc];
}

#if HAS_METALFX
void Engine::ensureScaler(size_t inputWidth, size_t inputHeight, size_t outputWidth, size_t outputHeight) {
    ...
    if (spatialScaler_ && scalerInputWidth_ == inputWidth && scalerInputHeight_ == inputHeight &&
        scalerOutputWidth_ == outputWidth && scalerOutputHeight_ == outputHeight && scaledOutputTexture_) {
        return;
    }
    ...
    if (@available(macOS 26.0, *)) {
        if ([MTLFXSpatialScalerDescriptor supportsMetal4FX:device_]) {
            NSLog(@"MetalGoose: Using Metal 4 enhanced MetalFX path");
        }
    }
    
    if (![MTLFXSpatialScalerDescriptor supportsDevice:device_]) {
        NSLog(@"MetalGoose: Device does not support MetalFX Spatial Scaler");
        spatialScaler_ = nil;
        return;
    }
    ...
    if (!spatialScaler_) {
        NSLog(@"MetalGoose: MTLFXSpatialScaler creation failed");
    } else {
        NSLog(@"MetalGoose: MetalFX Spatial Scaler created: %zux%zu -> %zux%zu", 
              inputWidth, inputHeight, outputWidth, outputHeight);
    }
}
#else
void Engine::ensureScaler(size_t inputWidth, size_t inputHeight, size_t outputWidth, size_t outputHeight) {
    (void)inputWidth; (void)inputHeight; (void)outputWidth; (void)outputHeight;
    spatialScaler_ = nil;
    scaledOutputTexture_ = nil;
}
#endif
...
id<MTLTexture> Engine::processFrame(id<MTLTexture> inputTexture, id<MTLCommandBuffer> commandBuffer) {
    auto startTime = std::chrono::high_resolution_clock::now();
    
    if (!inputTexture) return nil;
    ...
    if (config_.baseWidth == 0 && config_.renderScaleFactor > 0.0f && config_.renderScaleFactor != 1.0f) {
        baseWidth = static_cast<size_t>(static_cast<float>(inputWidth) / config_.renderScaleFactor);
    }
    if (config_.baseHeight == 0 && config_.renderScaleFactor > 0.0f && config_.renderScaleFactor != 1.0f) {
        baseHeight = static_cast<size_t>(static_cast<float>(inputHeight) / config_.renderScaleFactor);
    }
    size_t outputWidth = config_.outputWidth > 0 ? config_.outputWidth : baseWidth;
    size_t outputHeight = config_.outputHeight > 0 ? config_.outputHeight : baseHeight;
    
    bool needsResize = (outputWidth != inputWidth || outputHeight != inputHeight);
    if (!needsResize && config_.upscaleFactor > 1.0f && config_.upscaleMode != UpscaleMode::Off) {
        outputWidth = static_cast<size_t>(static_cast<float>(inputWidth) * config_.upscaleFactor);
        outputHeight = static_cast<size_t>(static_cast<float>(inputHeight) * config_.upscaleFactor);
        needsResize = true;
    }
        ...
        if (@available(macOS 26.0, *)) {
            if (useMetalFX && scaledOutputTexture_) {
                ...
                if (spatialScaler_) {
                    ...
                    bool applySharpening = (config_.upscaleMode == UpscaleMode::CAS || config_.sharpness > 0.35f);
                    if (applySharpening && casPipeline_) {
                        MTLTextureDescriptor* sharpDesc = [MTLTextureDescriptor 
                            ...
                            height:outputHeight
                            mipmapped:NO];
                        sharpDesc.usage = MTLTextureUsageShaderRead | MTLTextureUsageShaderWrite;
                        sharpDesc.storageMode = MTLStorageModePrivate;
                        id<MTLTexture> sharpTex = [device_ newTextureWithDescriptor:sharpDesc];
                        
                        if (sharpTex) {
                            id<MTLComputeCommandEncoder> sharpEncoder = [commandBuffer computeCommandEncoder];
                            if (sharpEncoder) {
                                sharpEncoder.label = config_.upscaleMode == UpscaleMode::CAS ? 
                                    @"MGUP-1 Quality CAS" : @"MGUP-1 Standard CAS";
                                
                                SharpenParams sharpParams;
                                sharpParams.sharpness = config_.sharpness;
                                sharpParams.radius = config_.upscaleMode == UpscaleMode::CAS ? 1.2f : 1.0f;
                                ...
                                [sharpEncoder setBytes:&sharpParams length:sizeof(SharpenParams) atIndex:0];
                                
                                MTLSize threadGroupSize = MTLSizeMake(16, 16, 1);
                                MTLSize gridSize = MTLSizeMake((outputWidth + 15) / 16, (outputHeight + 15) / 16, 1);
                                [sharpEncoder dispatchThreadgroups:gridSize threadsPerThreadgroup:threadGroupSize];
    ...
    if (config_.upscaleMode == UpscaleMode::Bilinear && config_.sharpness > 0.2f && casPipeline_) {
    ...
    float processingMs = std::chrono::duration<float, std::milli>(endTime - startTime).count();
    processingTime_.store(processingMs, std::memory_order_relaxed);
    
    return result;
}

id<MTLTexture> Engine::generateInterpolatedFrame(id<MTLTexture> prevTexture, id<MTLTexture> currTexture, 
                                                  float t, id<MTLCommandBuffer> commandBuffer) {
    if (!prevTexture || !currTexture) return nil;
    ...
    switch (config_.frameGenQuality) {
        case FrameGenQuality::Performance:
            return interpolateSimple(prevTexture, currTexture, t, commandBuffer);
            
        case FrameGenQuality::Balanced:
            return interpolateBalanced(prevTexture, currTexture, t, commandBuffer);
            
        case FrameGenQuality::Quality:
            ensureMotionTextures(width, height);
            ensurePyramidTextures(width, height);
            computeMotionVectorsPyramid(prevTexture, currTexture, commandBuffer);
            return interpolateWithMotion(prevTexture, currTexture, t, commandBuffer);
    }
    
    return nil;
}

void Engine::computeMotionVectors(id<MTLTexture> prevTex, id<MTLTexture> currTex, id<MTLCommandBuffer> commandBuffer) {
    if (!motionEstimationPipeline_ || !motionVectorTexture_ || !confidenceTexture_) return;
    ...
    params.textureSize = simd_make_uint2((uint32_t)currTex.width, (uint32_t)currTex.height);
    ...
    MTLSize threadGroupSize = MTLSizeMake(16, 16, 1);
    MTLSize gridSize = MTLSizeMake((currTex.width + 15) / 16, (currTex.height + 15) / 16, 1);
    [encoder dispatchThreadgroups:gridSize threadsPerThreadgroup:threadGroupSize];
    
    [encoder endEncoding];
    
    if (motionRefinementPipeline_) {
        id<MTLComputeCommandEncoder> refineEncoder = [commandBuffer computeCommandEncoder];
        if (refineEncoder) {
            ...
            [refineEncoder dispatchThreadgroups:gridSize threadsPerThreadgroup:threadGroupSize];
            [refineEncoder endEncoding];
        }
    }
}

void Engine::computeMotionVectorsPyramid(id<MTLTexture> prevTex, id<MTLTexture> currTex, id<MTLCommandBuffer> commandBuffer) {
    if (!pyramidDownsample2xPipeline_ || !pyramidMotionPipeline_ || !upsampleMotionPipeline_) {
    ...
    MTLSize gridL1 = MTLSizeMake((pyramidPrevLevel1_.width + 7) / 8, (pyramidPrevLevel1_.height + 7) / 8, 1);
    ...
    MTLSize gridL2 = MTLSizeMake((pyramidPrevLevel2_.width + 7) / 8, (pyramidPrevLevel2_.height + 7) / 8, 1);
    ...
    id<MTLComputeCommandEncoder> upsampleEncoder = [commandBuffer computeCommandEncoder];
    if (upsampleEncoder) {
        ...
        MTLSize gridFull = MTLSizeMake((currTex.width + 7) / 8, (currTex.height + 7) / 8, 1);
        [upsampleEncoder dispatchThreadgroups:gridFull threadsPerThreadgroup:threadGroupSize];
    ...
    if (motionRefinementPipeline_) {
        ...
        if (refineEncoder) {
            ...
            MTLSize gridFull = MTLSizeMake((currTex.width + 7) / 8, (currTex.height + 7) / 8, 1);
            [refineEncoder dispatchThreadgroups:gridFull threadsPerThreadgroup:threadGroupSize];
            [refineEncoder endEncoding];
        }
    }
}

id<MTLTexture> Engine::interpolateWithMotion(id<MTLTexture> prevTex, id<MTLTexture> currTex, float t, id<MTLCommandBuffer> commandBuffer) {
    if (!qualityPipeline_ || !outputTexture_) return nil;
...
id<MTLTexture> Engine::interpolateSimple(id<MTLTexture> prevTex, id<MTLTexture> currTex, float t, id<MTLCommandBuffer> commandBuffer) {
    if (!performancePipeline_ || !outputTexture_) return nil;
...
id<MTLTexture> Engine::interpolateBalanced(id<MTLTexture> prevTex, id<MTLTexture> currTex, float t, id<MTLCommandBuffer> commandBuffer) {
    if (!balancedPipeline_ || !outputTexture_) return nil;
...
id<MTLTexture> Engine::applyAntiAliasing(id<MTLTexture> inputTexture, id<MTLCommandBuffer> commandBuffer) {
    MTLSize threadGroupSize = MTLSizeMake(16, 16, 1);
    MTLSize gridSize = MTLSizeMake((inputTexture.width + 15) / 16, (inputTexture.height + 15) / 16, 1);
    ...
    switch (config_.aaMode) {
        ...
        case AAMode::FXAA: {
            if (!fxaaPipeline_ || !tempTexture_) return inputTexture;
            
            id<MTLComputeCommandEncoder> encoder = [commandBuffer computeCommandEncoder];
            if (!encoder) return inputTexture;
        ...
        case AAMode::SMAA: {
            if (!smaaEdgePipeline_ || !smaaWeightPipeline_ || !smaaBlendPipeline_) return inputTexture;
            if (!smaaEdgeTexture_ || !smaaWeightTexture_ || !outputTexture_) return inputTexture;
        ...
        case AAMode::MSAA: {
            if (!msaaPipeline_ || !tempTexture_) return inputTexture;
...
id<MTLTexture> Engine::applyTAA(id<MTLTexture> inputTexture, id<MTLCommandBuffer> commandBuffer) {
    if (!temporalPipeline_ || !historyTexture_ || !taaOutputTexture_) return inputTexture;
...
void Engine::copyTextureToHistory(id<MTLTexture> source, id<MTLCommandBuffer> commandBuffer) {
    if (!copyPipeline_ || !historyTexture_) return;
    ...
    MTLSize threadGroupSize = MTLSizeMake(16, 16, 1);
    MTLSize gridSize = MTLSizeMake((source.width + 15) / 16, (source.height + 15) / 16, 1);
    [encoder dispatchThreadgroups:gridSize threadsPerThreadgroup:threadGroupSize];
    [encoder endEncoding];
}

id<MTLTexture> Engine::applySharpening(id<MTLTexture> inputTexture, id<MTLCommandBuffer> commandBuffer) {
    if (!casPipeline_ || !outputTexture_) return nil;
...
Engine* Engine_Create(void* device, void* queue) {
    if (!device || !queue) {
        NSLog(@"Engine_Create: Invalid device or queue pointer");
        return nullptr;
    }
    id<MTLDevice> mtlDevice = (__bridge id<MTLDevice>)device;
    id<MTLCommandQueue> mtlQueue = (__bridge id<MTLCommandQueue>)queue;
    if (!mtlDevice || !mtlQueue) {
        NSLog(@"Engine_Create: Failed to bridge device or queue");
        return nullptr;
    }
...
void Engine_SetConfig(Engine* engine, void* configPtr) {
    ...
    DirectEngineConfigInternal* cfg = static_cast<DirectEngineConfigInternal*>(configPtr);
    
    EngineConfig engineConfig;
    engineConfig.targetFPS = static_cast<uint32_t>(cfg->targetFPS);
    engineConfig.frameGenEnabled = (cfg->frameGenMode > 0);
    engineConfig.frameGenMultiplier = static_cast<uint32_t>(cfg->frameGenMultiplier);
    ...
    engineConfig.baseWidth = cfg->baseWidth > 0 ? static_cast<uint32_t>(cfg->baseWidth) : 0;
    engineConfig.baseHeight = cfg->baseHeight > 0 ? static_cast<uint32_t>(cfg->baseHeight) : 0;
    engineConfig.outputWidth = cfg->outputWidth > 0 ? static_cast<uint32_t>(cfg->outputWidth) : 0;
    engineConfig.outputHeight = cfg->outputHeight > 0 ? static_cast<uint32_t>(cfg->outputHeight) : 0;
    
    switch (cfg->frameGenQuality) {
        case 0: engineConfig.frameGenQuality = FrameGenQuality::Performance; break;
        case 1: engineConfig.frameGenQuality = FrameGenQuality::Balanced; break;
        case 2: engineConfig.frameGenQuality = FrameGenQuality::Quality; break;
        default: engineConfig.frameGenQuality = FrameGenQuality::Balanced;
    }
    
    switch (cfg->aaMode) {
        case 0: engineConfig.aaMode = AAMode::Off; break;
        case 1: engineConfig.aaMode = AAMode::FXAA; break;
        case 2: engineConfig.aaMode = AAMode::SMAA; break;
        case 3: engineConfig.aaMode = AAMode::MSAA; break;
        case 4: engineConfig.aaMode = AAMode::TAA; break;
        default: engineConfig.aaMode = AAMode::Off;
    }
    ...
    switch (cfg->renderScale) {
        case 0: engineConfig.renderScaleFactor = 1.0f; break;
        case 1: engineConfig.renderScaleFactor = 0.75f; break;
        case 2: engineConfig.renderScaleFactor = 0.67f; break;
        case 3: engineConfig.renderScaleFactor = 0.50f; break;
        case 4: engineConfig.renderScaleFactor = 0.33f; break;
        default: engineConfig.renderScaleFactor = 1.0f; break;
    }
...
void* Engine_ProcessFrame(Engine* engine, void* inputTexture, void* cmdBuf) {
    ...
    id<MTLTexture> result = engine->processFrame((__bridge id<MTLTexture>)inputTexture, 
                                                  (__bridge id<MTLCommandBuffer>)cmdBuf);
    return (__bridge void*)result;
}

void* Engine_GenerateInterpolatedFrame(Engine* engine, void* prevTex, void* currTex, float t, void* cmdBuf) {
    if (!engine) return nullptr;
    id<MTLTexture> result = engine->generateInterpolatedFrame((__bridge id<MTLTexture>)prevTex,
                                                               (__bridge id<MTLTexture>)currTex,
                                                               t,
                                                               (__bridge id<MTLCommandBuffer>)cmdBuf);
    return (__bridge void*)result;
}
