    ...
    private func drawFrame(in view: MTKView, engine: DirectEngineRef) {
        ...
        if hasNew {
            if let processed = DirectEngine_ProcessFrame(engine, device, commandBuffer) {
                ...
                if frameDelta > 0 && frameDelta < 1.0 {
                    frameTimeAccumulator = frameTimeAccumulator * 0.7 + frameDelta * 0.3
                }
                
                if frameGenEnabled {
                    DirectEngine_SetTargetFrameMultiplier(engine, Int32(frameGenMultiplier))
                }
            }
        } else if frameGenEnabled {
            if DirectEngine_HasPendingAsyncFrame(engine) {
                if let asyncTex = DirectEngine_GetNextAsyncFrame(engine) {
                    outputTexture = asyncTex
            ...
            if outputTexture == nil {
                ...
                if frameGenType == .adaptive {
                    effectiveMultiplier = Int(DirectEngine_GetFrameGenMultiplier(engine))
                } else {
                    effectiveMultiplier = frameGenMultiplier
                }
                
                let timeSinceRealFrame = startTime - lastRealFrameTime
                let expectedFrameTime = 1.0 / Double(vsyncEnabled ? 60 : 120)
                
                frameGenCycle += 1
                
                if frameGenCycle < effectiveMultiplier {
                    let cycleProgress = timeSinceRealFrame / (frameTimeAccumulator > 0 ? frameTimeAccumulator : expectedFrameTime)
                    let t = Float(min(max(cycleProgress, 0.0), 1.0))
                    
                    var isInterpolated = false
                    if let interpTex = DirectEngine_GetInterpolatedFrameWithT(
                        engine, device, commandBuffer, t, &isInterpolated
                    ) {
